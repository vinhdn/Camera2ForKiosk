...
class CameraPreview : FrameLayout, CameraEvents {
    ...
    fun setPreviewListener(callback: PreviewCallback) {
        cameraApi.setPreviewListener {
            cameraApi.cameraHandler.post {
                callback.onPreview(it)
            }
        }
    }
    ...
    private suspend fun startPreview(): Unit = suspendCoroutine {
        ...
        if (surfaceTexture != null && attributes != null) {
            ...
            surfaceTexture.size =
                    if (CameraFacing.FRONT == cameraFacing && (Build.MODEL.startsWith("TPS") || Build.MODEL.startsWith("GM"))) {
                        when (previewOrientation % 180) {
                            0 -> CameraSize(previewSize.height, previewSize.width)
                            else -> previewSize
                        }
                    } else {
                        when (previewOrientation % 180) {
